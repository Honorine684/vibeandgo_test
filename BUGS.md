...
**Point d'attention non vérifié** : la fenêtre de 90-180s sur `/api/slow/stall` suppose qu'aucun proxy en amont (Render, Cloudflare) ne coupe une connexion HTTP inactive avant ce délai — les timeouts Node ont été explicitement désactivés/relevés côté serveur, mais un timeout imposé par la plateforme d'hébergement resterait hors de contrôle du code. Le scan de baseline dira si ça tient réellement 90-180s ou si ça coupe plus tôt.
